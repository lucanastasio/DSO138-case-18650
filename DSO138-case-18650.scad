// edited by Luca Anastasio <anastasio.lu@gmail.com>
//
//  case for DSO138
//
...
batt = [18.5, 67.0, 18.5]; // 9 V battery with contact
...
bncpos = 15.0-pcb[0]/2; // from upper left corner
...
switchneck = 5.0; // neck of toggle switch
...
module switchhole() {
	translate([pcb[0]/2+batt[0]/2+2*tol+wall, pcb[1]/2, -(pcb2bot+pcb[2]+pcb2lcd)+batt[2]/2]) {
		rotate([-90, 0, 0]) {
            translate([switchscrewdist, 0, 0]) cylinder(r=(switchscrew+tol)/2, h=tol+wall+d+1, $fn=20);
            translate([0, 0, (tol+wall+d+1)/2]) cube([switchrect[1], switchrect[0], tol+wall+d+1], center=true);
            translate([-switchscrewdist, 0, 0]) cylinder(r=(switchscrew+tol)/2, h=tol+wall+d+1, $fn=20);
		//hull () {
